...
function cursorColumnAfter(text, initialColumn, columns) {
  let column = initialColumn
  let pendingWrap = false
  for (const character of String(text).replace(ANSI_SEQUENCE, '')) {
    if (character === '\n' || character === '\r') {
      column = 0
      pendingWrap = false
      continue
...
export class LiveStatusBar {
  ...
  draw() {
    ...
    this.output.write(`\n${ANSI.clearLine}${styled}${ANSI.cursorUp}\u001b[${this.contentColumn + 1}G`)
    this.visible = true
  }

  clear() {
    if (!this.visible) return
    this.output.write(`${ANSI.cursorDown}${ANSI.clearLine}${ANSI.cursorUp}\u001b[${this.contentColumn + 1}G`)
    this.visible = false
    // Cursor positioning cancels a terminal's pending auto-wrap state. Materialize
    // that wrap after removing the reserved status row so the next chunk starts
    // where an uninterrupted stream would have placed it.
    if (this.pendingWrap) {
      this.output.write('\r\n')
      this.contentColumn = 0
      this.pendingWrap = false
  ...
  afterOutput(text = '') {
    const columns = finite(this.output.columns)
    const position = cursorColumnAfter(text, this.contentColumn, columns)
    this.contentColumn = position.column
    this.pendingWrap = position.pendingWrap
    this.draw()
  }
