...
function markerPrefixLength(text, marker) {
...
function encodePastedNewlines(text) {
  return text.replace(/\r\n|\r|\n/gu, PASTED_NEWLINE)
}

export function decodePastedNewlines(text) {
  return text.replaceAll(PASTED_NEWLINE, '\n')
}
...
export class BracketedPasteInput extends Transform {
  constructor(input) {
    ...
    if (typeof input.setRawMode === 'function') {
      this.setRawMode = input.setRawMode.bind(input)
    }
    input.pipe(this)
  }
  ...
  _flush(callback) {
    try {
      ...
      if (this.pending !== '') this.push(this.inPaste ? encodePastedNewlines(this.pending) : this.pending)
      this.pending = ''
      callback()
    } catch (error) {
      callback(error)
  ...
  process(chunk, flush = false) {
    ...
    while (text !== '') {
      ...
      if (index >= 0) {
        ...
        if (before !== '') this.push(this.inPaste ? encodePastedNewlines(before) : before)
        text = text.slice(index + marker.length)
        this.inPaste = !this.inPaste
        continue
      }
      const retained = flush ? 0 : markerPrefixLength(text, marker)
      const complete = retained === 0 ? text : text.slice(0, -retained)
      if (complete !== '') this.push(this.inPaste ? encodePastedNewlines(complete) : complete)
      this.pending = retained === 0 ? '' : text.slice(-retained)
      break
    }
  }
...
export class TerminalUI {
  constructor({ input = process.stdin, output = process.stdout, error = process.stderr, color = true, liveStatus } = {}) {
    ...
    this.readlineInput = this.terminal ? new BracketedPasteInput(input) : input
    if (this.terminal) output.write(BRACKETED_PASTE_ENABLE)
  ...
  write(stream, text) {
    ...
    stream.write(text)
    this.liveStatus?.afterOutput(text)
  }
  ...
  async question(prompt, signal) {
    ...
    try {
      const answer = await this.readline.question(prompt, signal === undefined ? undefined : { signal })
      return decodePastedNewlines(answer)
    } finally {
  ...
  close() {
    ...
    if (this.terminal) {
      ...
      this.readlineInput.dispose()
    }
  }

  banner({ cwd, model, provider, permissionMode, sessionId, endpoint }) {
    ...
    if (endpoint !== undefined) this.line(`${this.style('端点', 'gray')}      ${endpoint}`)
  ...
  async askEndpointSelection({ endpoints, currentName }) {
    this.line()
    this.line(this.style('选择 API 端点', 'bold'))
    if (endpoints.length === 0) {
      this.errorLine('没有已配置的命名端点。')
      return undefined
    }
    endpoints.forEach((endpoint, index) => {
      const selected = endpoint.name === currentName
      const description = endpoint.description ? ` — ${endpoint.description}` : ''
      this.line(`  ${index + 1}. ${selected ? '✓ ' : '  '}${endpoint.name}${this.style(description, 'dim')}`)
      this.line(`       ${this.style(endpoint.baseURL, 'dim')}`)
    })
    const raw = (await this.question(this.style('输入编号（直接回车取消）：', 'cyan'))).trim()
    if (raw === '') return undefined
    const endpoint = endpoints[Number(raw) - 1]
    if (endpoint === undefined || !/^\d+$/u.test(raw)) {
      this.errorLine('端点编号无效。')
      return undefined
    }
